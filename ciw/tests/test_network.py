...
class TestNetwork(unittest.TestCase):
    ...
    def test_create_network_from_dictionary(self):
        ...
        params = {
            ...
            "number_of_servers": [
                ciw.Schedule(numbers_of_servers=[1, 4], shift_end_dates=[20, 50]),
                3,
            ],
        ...
        self.assertEqual(N.service_centres[0].class_change_matrix, None)
        self.assertEqual(
            N.service_centres[0].number_of_servers.shift_end_dates, [20, 50]
        )
        self.assertEqual(
            N.service_centres[0].number_of_servers.numbers_of_servers, [1, 4]
        )
        ...
        self.assertEqual(
            [round(p, 2) for p in router.routers[0].probs], [0.5, 0.2, 0.3]
        )
        self.assertEqual(router.routers[1].destinations, [1, 2, -1])
        self.assertEqual(
            [round(p, 2) for p in router.routers[1].probs], [0.0, 0.0, 1.0]
        )
        ...
        self.assertEqual(
            ...
            [
                "Exponential(rate=7.0)",
                "Uniform(lower=0.4, upper=1.2)",
                "Deterministic(value=5.33)",
            ],
        )
        router = N.customer_classes["Customer"].routing
        self.assertEqual(router.routers[0].destinations, [1, 2, 3, -1])
        self.assertEqual(
            [round(p, 2) for p in router.routers[0].probs], [0.5, 0.0, 0.1, 0.4]
        )
        ...
        self.assertEqual(
            [round(p, 2) for p in router.routers[2].probs], [0.0, 0.0, 0.0, 1.0]
        )
        self.assertEqual(
    ...
    def test_raising_errors(self):
        ...
        self.assertRaises(
            ValueError, ciw.create_network_from_dictionary, params_list[21]
        )
        params_list[22]["class_change_time_distributions"] = {
            "Class 0": {"Class 0": None},
            "Class 1": {"Class 0": None, "Class 1": None},
        }
        self.assertRaises(
            ValueError, ciw.create_network_from_dictionary, params_list[22]
        )
