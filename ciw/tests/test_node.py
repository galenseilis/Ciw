...
class TestNode(unittest.TestCase):
    ...
    def test_date_from_schedule_generator(self):
        sg = ciw.Schedule(
            numbers_of_servers=[1, 0, 2, 3], shift_end_dates=[30, 60, 90, 100]
        )
        sg.initialise()
        sg.get_next_shift()
    ...
    def test_server_utilisation_with_schedules(self):
        N = ciw.create_network(
            ...
            number_of_servers=[
                ciw.Schedule(numbers_of_servers=[1, 2], shift_end_dates=[9, 23])
            ],
        )
        Q = ciw.Simulation(N)
    ...
    def test_reneging_with_schedules(self):
        N = ciw.create_network(
            ...
            service_distributions=[ciw.dists.Deterministic(11)],
            number_of_servers=[
                ciw.Schedule(numbers_of_servers=[1, 0], shift_end_dates=[16, 10000])
            ],
            reneging_time_distributions=[ciw.dists.Deterministic(3)],
        )
        Q = ciw.Simulation(N)
    ...
    def test_preemptive_priorities_resume_options_due_to_schedule(self):
        ...
        N = ciw.create_network(
            ...
            number_of_servers=[
                ciw.Schedule(numbers_of_servers=[1, 2], shift_end_dates=[5, 100])
            ],
            priority_classes=({"Class 0": 0, "Class 1": 1}, ["restart"]),
        )
        Q = ciw.Simulation(N)
    ...
    def test_shift_change_before_arrival(self):
        N = ciw.create_network(
            ...
            number_of_servers=[
                ciw.Schedule(
                    numbers_of_servers=[1, 2],
                    shift_end_dates=[1.0, 10.0],
                    preemption=False,
                )
            ],
        )
